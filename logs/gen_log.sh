...
#  Note: This command often requires root privileges (run as root, or use `sudo`)
network_interface=wlp3s0
tcpdump -n -i $network_interface -C 100 -W 10 -w ${network_interface}.pcap -v

#With tshark we can convert the pcap file into ascii:
# tshark -r wlp3s0.pcap0 > wlp3s0.pcap0.ascii
